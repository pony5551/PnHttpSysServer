unit uPnHttpSysServer_PnSockString;
...
type
  ...
  private
    ...
    fOutContent,
    fOutContentType,
    fOutCustomHeaders: TPnSockString;
  ...
  public
    ...
    /// URI与请求参数
    property URL: TPnSockString read fURL;
    /// 请求方法(GET/POST...)
    property Method: TPnSockString read fMethod;
    /// 请求头
    property InHeaders: TPnSockString read fInHeaders;
    /// 请求body,多为post或上传文件
    property InContent: TPnSockString read fInContent;
    /// post长度
    ...
    property InContentType: TPnSockString read fInContentType;
    //Response
    /// 输出body文本,发送内存流时用到，大小受string限制为2G,为静态文件时此处为文件路径
    property OutContent: TPnSockString read fOutContent write fOutContent;
    /// 响应body的内容类型,指定HTTP_RESP_STATICFILE交给SendResponse处理静态文件
    // - 为!NORESPONSE时可自定义类型,如处理websockets消息等等
    property OutContentType: TPnSockString read fOutContentType write fOutContentType;
    /// 响应头信息
    property OutCustomHeaders: TPnSockString read fOutCustomHeaders write fOutCustomHeaders;
    /// http statuscode
...
begin
  ...
  fInHeaders.FLen := 0;
  fInContent.FLen := 0;
  fInContentType.FLen := 0;

  fRemoteIP.FLen := 0;
  fInContentEncoding.FLen := 0;
  fOutCustomHeaders.FLen := 0;
...
begin
  ...
  fURL.InitAndClean(256);
  fMethod.InitAndClean(64);
  fInHeaders.InitAndClean(2048);
  fInContent.InitAndClean(64);
  fInContentType.InitAndClean(256);

  fInFileUpload := False;

  fRemoteIP.InitAndClean(64);
  //fRemoteIP := '';

  fInContentLength := 0;
  fInContentLengthRead := 0;
  fInContentEncoding.InitAndClean(256);
  //fInContentEncoding := '';
  //fInAcceptEncoding := '';
  //fInRange := '';
  ...
  fOutContent.InitAndClean(64);
  fOutContentType.InitAndClean(256);
  fOutCustomHeaders.InitAndClean(64);
  fOutStatusCode := 200;
  fConnectionID := 0;
  fUseSSL := False;
...
begin
  ...
  SetHeaders(@fResp,pointer(fOutCustomHeaders.FValue),Heads);
  ...
  if fOutContentType.FValue=HTTP_RESP_STATICFILE then
  begin
    SendFile(fOutContent.FValue, '', Heads, pLogFieldsData);
  end
  else begin
    // response is in OutContent -> send it from memory
    if fOutContentType.FValue=HTTP_RESP_NORESPONSE then
      ...
      with fResp.Headers.KnownHeaders[HttpHeaderContentEncoding] do
        ...
        begin
          ...
          OutContentEncoding := CompressDataAndGetHeaders(fInCompressAccept,
            fServer.fCompress,fOutContentType.FValue,fOutContent);
          pRawValue := pointer(OutContentEncoding);
          RawValueLength := Length(OutContentEncoding);
        end;
    end;

    if fOutContent.FValue<>'' then
    begin
      dataChunk.DataChunkType := HttpDataChunkFromMemory;
      dataChunk.pBuffer := PAnsiChar(fOutContent.FValue);
      dataChunk.BufferLength := fOutContent.Length;
...
begin
  ...
  if AMimeType<>'' then
    ...
    fOutContentType := GetMimeContentType(nil,0,fOutContent.FValue);
  with fResp do
  begin
    //ContentType
    Headers.KnownHeaders[HttpHeaderContentType].RawValueLength := fOutContentType.Length;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          with AContext do
            hr := OnBeforeBody(fURL.FValue,fMethod.FValue,fInHeaders.FValue,fInContentType.FValue,fRemoteIP.FValue,fInContentLength,fUseSSL);
        ...
        begin
          ...
          if IdemPChar(PAnsiChar(fInContentType.FValue),'multipart/form-data') then
            fInFileUpload := True;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            //数据已接收完成
            //gzip解码
            if fInContentEncoding.FValue<>'' then
              for i := 0 to high(fCompress) do
                if fCompress[i].Name=fInContentEncoding.FValue then
                begin
      ...
      begin
        ...
        //数据指针
        fInContentBufRead := PAnsiChar(PAnsiChar(fInContent.FValue)+fInContentLengthRead);
      end;
      BytesRead := 0;
