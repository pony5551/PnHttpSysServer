unit uPnHttpSys.Comm_PnSockString;
...
type
  PPnSockString = ^TPnSockString;
  TPnSockString = record
  private
  ...
  public
    FValue: SockString;
    FLen: Integer;  //物理长度
    FRealLen: Integer;  //实际长度
    class operator Implicit(const S: AnsiString): TPnSockString;
    function From(p: PAnsiChar; ALen: Integer): PPnSockString;
...
function RetrieveHeaders(const Request: HTTP_REQUEST;
  const RemoteIPHeadUp: SockString; out RemoteIP: TPnSockString): SockString;
...
function CompressDataAndGetHeaders(Accepted: THttpSocketCompressSet;
  ...
  var OutContent: TPnSockString): SockString;
...
{ TPnSockString }
function TPnSockString.GetLength: Integer;
begin
  Result := FRealLen;
end;

procedure TPnSockString.SetLength(const nValue: Integer);
begin
  if nValue>FLen then
  begin
    FLen := nValue;
    FRealLen := nValue;
    System.SetLength(FValue, FLen);
  end
  else begin
    FRealLen := nValue;
  end;
...
class operator TPnSockString.Implicit(const S: AnsiString): TPnSockString;
begin
  Result.From(PAnsiChar(S), System.Length(S));
end;

function TPnSockString.From(p: PAnsiChar; ALen: Integer): PPnSockString;
begin
  SetLength(ALen);
...
procedure TPnSockString.InitAndClean(ALen: Integer);
begin
  if FLen<ALen then
    SetLength(ALen);
  FillChar(PAnsiChar(FValue)^, FLen, 0);
end;
...
procedure CompressInternal(var Data: TPnSockString; Compress, ZLib: boolean);
var tmp: ZipString;
    DataLen: integer;
...
//    Data.Length := DataLen+256+DataLen shr 3;
//    DataLen := CompressMem(pointer(tmp),pointer(Data.FValue),DataLen,Data.Length,
//      HTTP_LEVEL,ZLib);
//    if DataLen<=0 then
//      Data := ''
//    else
...
//      Data.Length := DataLen;
    ...
    //UnCompressZipString(pointer(tmp),DataLen,ZipString(Data.FValue),nil,ZLib,0);
    LStream := TMemoryStream.Create;
    try
...
function CompressDeflateEx(var DataRawByteString; Compress: boolean): AnsiString;
var Data: TPnSockString absolute DataRawByteString;
begin
...
procedure IP4Text(const ip4addr; var result: TPnSockString);
...
procedure GetSinIPFromCache(const sin: TVarSin; var result: TPnSockString);
begin
...
begin
  ...
  if (RemoteIP.FValue='') and (Request.Address.pRemoteAddress<>nil) then
