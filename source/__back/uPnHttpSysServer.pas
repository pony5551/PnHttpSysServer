...
type
  ...
  private
    ...
    fInHeaders,
    fInContent,
    fInContentType: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF};
    fInFileUpload: Boolean;
    fInContentBufRead: PAnsiChar;
    fRemoteIP: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF};

    fInContentLength,
    fInContentLengthRead: UInt64;
    //fInContentLengthChunk: Cardinal;
    fInContentEncoding: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF};
    ...
    fOutContent,
    fOutContentType,
    fOutCustomHeaders: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF};
  ...
  public
    ...
    /// URI与请求参数
    property URL: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF} read fURL;
    /// 请求方法(GET/POST...)
    property Method: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF} read fMethod;
    /// 请求头
    property InHeaders: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF} read fInHeaders;
    /// 请求body,多为post或上传文件
    property InContent: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF} read fInContent;
    /// post长度
    ...
    property InContentType: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF} read fInContentType;
    //Response
    /// 输出body文本,发送内存流时用到，大小受string限制为2G,为静态文件时此处为文件路径
    property OutContent: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF} read fOutContent write fOutContent;
    /// 响应body的内容类型,指定HTTP_RESP_STATICFILE交给SendResponse处理静态文件
    // - 为!NORESPONSE时可自定义类型,如处理websockets消息等等
    property OutContentType: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF} read fOutContentType write fOutContentType;
    /// 响应头信息
    property OutCustomHeaders: {$IFDEF PNSTRING}TPNString{$ELSE}SockString{$ENDIF} read fOutCustomHeaders write fOutCustomHeaders;
    /// http statuscode
...
begin
  ...
  fInHeaders.Len := 0;
  fInContent.Len := 0;
  fInContentType.Len := 0;

  fRemoteIP.Len := 0;
  fInContentEncoding.Len := 0;
  fOutCustomHeaders.Len := 0;
  {$ELSE}
  fURL := '';
  fMethod := '';
  fInHeaders := '';
...
begin
  ...
  {$IFDEF PNSTRING}
  fURL.InitAndClean(16);
  fMethod.InitAndClean(16);
  fInHeaders.InitAndClean(16);
  fInContent.InitAndClean(16);
  fInContentType.InitAndClean(16);
  {$ELSE}
  fURL := '';
  fMethod := '';
  fInHeaders := '';
  ...
  {$IFDEF PNSTRING}
  fInContentEncoding.InitAndClean(16);
  {$ELSE}
  fInContentEncoding := '';
  {$ENDIF}
  //fInContentEncoding := '';
  ...
  {$IFDEF PNSTRING}
  fOutContent.InitAndClean(16);
  fOutContentType.InitAndClean(16);
  fOutCustomHeaders.InitAndClean(16);
  {$ELSE}
  fOutContent := '';
  fOutContentType := '';
  fOutCustomHeaders := '';
...
begin
  ...
  AStream.Read(PAnsiChar(fOutContent.FValue)^, AStream.Size);
  {$ELSE}
  ...
  try
    ...
    begin
      ...
      begin
        ...
        {$IFDEF PNSTRING}
        ClientIp := PAnsiChar(Self.fRemoteIP.FValue);
        ClientIpLength := Self.fRemoteIP.Length;
        Method := pointer(Self.fMethod.FValue);
        MethodLength := Self.fMethod.Length;
        {$ELSE}
        ClientIp := PAnsiChar(Self.fRemoteIP);
    ...
    {$IFDEF PNSTRING}
    dataChunk.pBuffer := PAnsiChar(fOutContent.FValue);
...
begin
  ...
  SetHeaders(@fResp,pointer(fOutCustomHeaders{$IFDEF PNSTRING}.FValue{$ENDIF}),Heads);
  ...
  if fOutContentType{$IFDEF PNSTRING}.FValue{$ENDIF}=HTTP_RESP_STATICFILE then
  begin
    SendFile(fOutContent{$IFDEF PNSTRING}.FValue{$ENDIF}, '', Heads, pLogFieldsData);
  end
  else begin
    // response is in OutContent -> send it from memory
    if fOutContentType{$IFDEF PNSTRING}.FValue{$ENDIF}=HTTP_RESP_NORESPONSE then
      ...
      with fResp.Headers.KnownHeaders[HttpHeaderContentEncoding] do
        ...
        begin
          ...
          OutContentEncoding := CompressDataAndGetHeaders(fInCompressAccept,
            fServer.fCompress,fOutContentType{$IFDEF PNSTRING}.FValue{$ENDIF},fOutContent);
          pRawValue := pointer(OutContentEncoding);
          RawValueLength := Length(OutContentEncoding);
        end;
    end;

    if fOutContent{$IFDEF PNSTRING}.FValue{$ENDIF}<>'' then
    begin
      dataChunk.DataChunkType := HttpDataChunkFromMemory;
      {$IFDEF PNSTRING}
      dataChunk.pBuffer := PAnsiChar(fOutContent.FValue);
      ...
      begin
        ...
        Headers.KnownHeaders[HttpHeaderContentType].pRawValue := PAnsiChar(fOutContentType.FValue);
        {$ELSE}
...
begin
  ...
  if AMimeType<>'' then
    ...
    fOutContentType := GetMimeContentType(nil,0,fOutContent{$IFDEF PNSTRING}.FValue{$ENDIF});
  with fResp do
  begin
    //ContentType
    {$IFDEF PNSTRING}
  ...
  begin
    ...
    begin
      ...
      if fReq^.Verb in [low(Verbs)..high(Verbs)] then
        ...
        {$IFDEF PNSTRING}
        fMethod.From(fReq^.pUnknownVerb,fReq^.UnknownVerbLength);
        {$ELSE}
      ...
      begin
        {$IFDEF PNSTRING}
        fInContentType.From(pRawValue,RawValueLength);
        {$ELSE}
      ...
      begin
        ...
        begin
          {$IFDEF PNSTRING}
          fInContentEncoding.From(pRawValue,RawValueLength);
          {$ELSE}
        ...
        begin
          ...
          if IdemPChar(PAnsiChar(fInContentType{$IFDEF PNSTRING}.FValue{$ENDIF}),'multipart/form-data') then
            fInFileUpload := True;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            //数据已接收完成
            //gzip解码
            if fInContentEncoding{$IFDEF PNSTRING}.FValue{$ENDIF}<>'' then
              for i := 0 to high(fCompress) do
                if fCompress[i].Name=fInContentEncoding{$IFDEF PNSTRING}.FValue{$ENDIF} then
                begin
      ...
      begin
        ...
        //数据指针
        fInContentBufRead := PAnsiChar(PAnsiChar(fInContent{$IFDEF PNSTRING}.FValue{$ENDIF})+fInContentLengthRead);
      end;
      BytesRead := 0;
